...
namespace Designer.Controllers.App.Commands {
	public class AddShapeRectangleCommand : ICommand {
		...
		public AddShapeRectangleCommand(ShapeMapper shapeMapper, GroupShape shapeGroup, Vector2 position, Vector2 size) {
		...
		public void Execute() {
			this.createdShape = new ShapeWrapper(
				new BaseShape(position, size, ShapeStrategyProvider.Instance.GetRectangleStrategy())
			);
